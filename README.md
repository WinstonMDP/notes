...
Приложение и БД спроектированы так, чтобы операции были атомарными, и выполнялись независимо от других, поэтому в большинстве случаев группировки запросов в транзакции не требуется. Но при удалении пользователя может возникнуть ситуация, что он был единственным членом каких-то групп (то есть они были созданы им самим, и никого кроме него там нет). В таком случае после удаления пользователя необходимо удалить еще и группы, которые остались пустыми. Сделать это можно с помощью такой транзакции:
